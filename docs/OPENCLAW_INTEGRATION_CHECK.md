...
2. **Claude runtime phase (tool)** — derived from actual stream-json events, emitted as concise bracketed labels: `[Working: using Bash…]` on tool_use start (reports the real tool name), `[Working: waiting for Bash, 12s…]` after 8s silence with a tool in flight. The raw `Agent` tool gets a deterministic funny subagent name plus optional activity extracted from tool input: `[Working: using Sir Greps-a-Lot — inspect auth flow…]` / `[Working: waiting for Sir Greps-a-Lot — inspect auth flow, 12s…]`. Names come from a small built-in whimsical list and are deterministic per tool-call id. Activity is extracted from fields like `description`, `subagent_type`, `prompt`, etc. and sanitized/truncated to a concise one-line display. Other tool names (Read, Bash, Write, etc.) are displayed unchanged. Deduplicated: the same phase is never emitted twice.
3. **Claude runtime phase (thinking)** — inferred, conservative: when the main agent is silent with no tool active for ≥8s, emits `[Working: thinking…]` once per silent period. Suppressed as soon as any tool or text activity appears. Lower priority than tool phases; does not override real activity.
