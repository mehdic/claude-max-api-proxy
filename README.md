...
- **3-layer keepalive/progress** (eager handshake → visible truthful progress → periodic SSE comment) keeps HTTP/SSE transports warm without fabricating invisible assistant text. When real progress is available, the proxy emits explicit bracketed visible chunks with a random clean status prefix such as `[Thinking: using Bash…]`, `[Tinkering: waiting for Bash, 12s…]`, `[Checking: thinking…]`, or `[n8n: workflow · 9s elapsed · exec 73]`. Only the prefix varies; the body (`using Bash`, `waiting for Bash`, `thinking`) remains derived from real runtime events or conservative silence inference, and dedupe uses the semantic phase rather than the rendered prefix. The raw Claude `Agent` tool gets special treatment: each call is assigned a deterministic funny subagent name (e.g. "Sir Greps-a-Lot", "Baron von Stacktrace") and the visible label always includes `Subagent`; when available, a short activity is extracted from the tool input — `[Thinking: using Sir Greps-a-Lot Subagent — inspect auth flow…]` / `[Tinkering: waiting for Sir Greps-a-Lot Subagent — inspect auth flow, 12s…]`. Names are deterministic per tool-call id (same call always gets the same name). Other tool names (Read, Bash, etc.) are displayed unchanged. When the main agent is silent with no tool active for ≥8s, the proxy conservatively infers a thinking phase and emits it once per silent period — this is suppressed as soon as any tool or text activity appears, and is lower priority than all other progress labels. Each visible progress chunk is wrapped with leading and trailing newlines (e.g. `\n[Thinking: using Bash…]\n`) so it renders on its own line, separated from surrounding model content. Generic idle keepalives remain transport-only SSE comments.
